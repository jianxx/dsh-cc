...
`command-plugin`, `command-mcp`, and — added here — `cc-model-routes`
(`@jianxx/dsh-cc-model-aliases`) and `tool-task` (`@jianxx/dsh-cc-subagent-task`).
...
| Subagents / Agent tool / teams          | 🔶        | `dsh-subagent*` providers + `tool-subagent-control` (`send_message`/`interrupt`/`list_agents`); CC `.claude/agents` now really dispatched via `@jianxx/dsh-cc-subagent-task` (CC `Task`, tool row `tool-task`) — `subagent_type` dispatch over per-workspace `.claude/agents` definitions: persona = definition systemPrompt, task text = child's first user message, model alias routed through the `ccModelRoutes` service, sanitized toolFilter, `maxDepth` 3; an `Available subagents` system-prompt section lists the options. **Known limits (v1): foreground one-shot only** (the `tool-subagent-fork` `backgroundMode: continuable` / `report`/`send_message` workflow is a follow-up), fork **inherits the parent prefix** (CC `Task` does not), **no seam plugin-agent dispatch** (`subagent_type` only addresses file definitions), and discovery is a **process-level cache** — editing `.claude/agents` needs a restart (mtime invalidation pending). Unknown `subagent_type` errors with the available list. The harness `tool-subagent`/`tool-subagent-fork` rows are disabled in the cc preset in favour of `tool-task`.                                                          |
