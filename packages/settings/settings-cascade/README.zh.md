...
| `localSettingsPath` | 本地（被 git 忽略的）设置文件 | `<project>/.claude/settings.local.json` |
| `flagSettingsPath` | 命令行 `--settings` 文件 | 无 |
| `flagSettingsInline` | 内联 `--settings` 内容，在 flag 文件之上合并 | 无 |
| `projectDir` | 默认项目/本地路径所用的项目根 | 当前工作目录 |
...
- **提供方是只读组合。** `writable` 为 `false`，seam 的进程内 `update()`/`replace()`/`mutate()` 路径被拒绝；namespace 经由各自叶子提供方写入。
...
间接生效：本提供方只组合并发布 namespace 分节，任何模型效果都经由 `ctx.settings` 的消费方产生，并由各消费方自己的接口文档说明。
...
- **只读且仅 JSON 来源。** 级联仅组合既有文档；叶子写入不在范围内。来源必须是 `.json`（settings.json 惯例）；YAML 及写入用户/项目文件暂缓实现。
