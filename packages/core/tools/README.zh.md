...
- `ctx.tools.reserve(name: string): () => void`：在调用层预留一个能力**名字**而不注册可见定义。该名字进入「已知但可限制」集合——某个作用域稍后可以对其 `restrict()`，`toolOrder` 也可以列出它——但在真正 `register()` 提供定义之前，它绝不会进入模型可见的 schema。这让延迟工具注册表能预置尚可在重定义加载前被限制的名字（[`dsh-tool-search`](../tool-search/README.md)）。
- `ctx.tools.isAdmitted(name: string, scope?: ScopeKey): boolean`：某个全局能力名字是否通过查看作用域链上的每一条作用域限制，与是否已注册无关。被某条 `allow` 列表排除、或出现在某条 `deny` 列表中的名字不被接纳；限制跨整条链取交集，与注册可见性完全一致。用于判断某个延迟能力是否可针对某个 agent 加载。
