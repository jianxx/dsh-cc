...
- **Isolated service realms.** Service-bearing rows (tool search, microcompactor, plugin registry, MCP connections, hook-bridge status) live inside the `cc-services` group with the five required `isolate` keys, so they publish into an entry-local realm instead of the process-global root realm (which would trip the preset mount gate).
