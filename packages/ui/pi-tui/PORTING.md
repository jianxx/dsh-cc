...
- **D4 (masked Input)**: `components/input.ts` gains a `masked` public field (default `false`); when set, `render()` displays one `•` per grapheme instead of the raw characters (secrets/API-key entry, `/provider` command). Display-only — raw value, submit, and all editing behavior unchanged; upstream `src/` bytes otherwise intact.
