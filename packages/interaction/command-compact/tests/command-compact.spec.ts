...
/**
 * Stub engine that records the hint parked on the agent at compactNow entry
 * (proving the command set it BEFORE dispatching) and appends the durable
 * compaction marker trio like the real engine does.
 */
...
describe('/compact human command', () => {
  it('reports success with useful accounting and forwards the exact target and signal', async () => {
    ...
    // No hint parked: the finally-block take cleared any remainder.
    expect(takeCompactHint(test.agent)).toBeUndefined()
  })

  it('forwards a non-empty argument as the preservation hint and clears it afterwards', async () => {
    const test = await harness()
    const execution = await run(test, ' keep the migration plan')
    expect(execution.result).toEqual({
      kind: 'success',
      text: 'Compacted 3 history items (~42 tokens).',
      sourceEventSeq: RESULT.summarySeq,
    })
    // The hint was parked BEFORE compactNow ran (the stub read it there).
    expect(test.compact.hintsAtCall).toEqual(['keep the migration plan'])
    // The finally-block take consumed the remainder.
    expect(takeCompactHint(test.agent)).toBeUndefined()
  })

  it('returns a direct no-history result and clears a parked hint even then', async () => {
    const test = await harness()
    test.compact.result = null
    const empty = await run(test, ' keep something')
    expect(empty.result).toEqual({
      kind: 'success',
      text: 'No compactable history yet.',
    })
    expect(test.compact.hintsAtCall).toEqual(['keep something'])
    expect(takeCompactHint(test.agent)).toBeUndefined()
  })
  ...
  it('clears a parked hint even when the backend throws an unexpected error', async () => {
    const test = await harness()
    const bug = new Error('unexpected backend bug')
    test.compact.failure = bug
    await expect(run(test, ' keep this')).rejects.toBe(bug)
    // The finally-block take ran despite the rejection.
    expect(takeCompactHint(test.agent)).toBeUndefined()
  })

  it('preserves cancellation as a direct error result', async () => {
    const test = await harness()
    const controller = new AbortController()
  ...
  it('drains an in-flight handler before plugin disposal settles', async () => {
    const test = await harness()
    const controller = new AbortController()
    const abort = new Error('operator cancelled')
...
describe('hint store interaction', () => {
  it('setCompactHint parks per agent; the command lifecycle consumes it', () => {
    const a = { id: 'a' }
    const b = { id: 'b' }
    setCompactHint(a, 'only a')
